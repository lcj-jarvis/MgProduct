...
public class BizSalesReportConsumeListener implements MessageListener {
    public BizSalesReportConsumeListener(StoreService storeService) {
    ...
    public ConsumerStatus consume(FaiMqMessage message, ConsumeContext consumeContext) {
        ...
        int aid = recvInfo.getInt(BizSalesReportEntity.Info.AID, 0);
        int unionPriId = recvInfo.getInt(BizSalesReportEntity.Info.UNION_PRI_ID, 0);
        int pdId = recvInfo.getInt(BizSalesReportEntity.Info.PD_ID, 0);
        if(aid == 0 || unionPriId == 0 || pdId == 0){
            return ConsumerStatus.CommitMessage;
        }
        int rt = m_storeService.reportBizSales(flow, aid, unionPriId, pdId);
