...
public class ProductBasicService extends MgProductInfService {
     ...
     *  修改商品数据 包括 规格、库存
     */
    public int setProductInfo(FaiSession session, int flow, int aid, int tid, int siteId, int lgId, int keepPriId1, Integer rlPdId, ParamUpdater recvUpdater) throws IOException {
        int rt = Errno.ERROR;
        Oss.SvrStat stat = new Oss.SvrStat(flow);
        try {
            if (!FaiValObj.TermId.isValidTid(tid)) {
            ...
            if (!Str.isEmpty(basicData)) {
                ...
                if (!Util.isEmptyList(delRlGroupIds) || !Util.isEmptyList(addRlGroupIds)) {
                    ProductBasicProc basicProc = new ProductBasicProc(flow);
                    rt = basicProc.setPdBindGroup(aid, unionPriId, rlPdId, addRlGroupIds, delRlGroupIds);
                    if (rt != Errno.OK) {
                        return rt;
                    }
    ...
    public int addProductInfo(FaiSession session, int flow, int aid, int ownerTid, int ownerSiteId, int ownerLgId, int ownerKeepPriId1, Param addInfo, Param inStoreRecordInfo) throws IOException {
        int rt = Errno.ERROR;
        Oss.SvrStat stat = new Oss.SvrStat(flow);
        try {
            // 获取 unionPriId
