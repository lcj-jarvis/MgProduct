...
public class MgProductStoreCli extends MgProductInternalCli {
     ...
     */
    public int reportSummary(int aid, FaiList<Integer> pdIds, FaiList<Long> skuIds, boolean reportCount, boolean reportPrice){
        m_rt = Errno.ERROR;
        Oss.CliStat stat = new Oss.CliStat(m_name, m_flow);
        try {
            if (aid == 0) {
            ...
            if(pdIds == null && skuIds == null){
                m_rt = Errno.ARGS_ERROR;
                Log.logErr(m_rt, "infoList error");
                return m_rt;
            }
            if(!reportCount && !reportPrice) {
                m_rt = Errno.ARGS_ERROR;
                Log.logErr(m_rt, "args err, report null;aid=%d;pdIds=%s;skuIds=%s;reportCount=%s;reportPrice=%s;", aid, pdIds, skuIds, reportCount, reportPrice);
                return m_rt;
            }
            ...
            FaiBuffer sendBody = new FaiBuffer(true);
            if(pdIds != null) {
                pdIds.toBuffer(sendBody, StoreSalesSkuDto.Key.PD_ID);
            }
            if(skuIds != null) {
                skuIds.toBuffer(sendBody, StoreSalesSkuDto.Key.SKU_ID);
            }
            sendBody.putBoolean(StoreSalesSkuDto.Key.REPORT_COUNT, reportCount);
     ...
     * 修改商品规格库存销售sku
     */
    public int batchSetSkuStoreSales(int aid, String xid, int tid, int ownerUnionPriId, FaiList<Integer> uidList, int pdId, int rlPdId, int sysType, FaiList<ParamUpdater> updaterList, FaiList<Integer> softDelUnionPriIds){
        m_rt = Errno.ERROR;
        Oss.CliStat stat = new Oss.CliStat(m_name, m_flow);
        try {
            if (aid == 0) {
            ...
            softDelUnionPriIds.toBuffer(sendBody, StoreSalesSkuDto.Key.SOFT_DEL_UNION_PRI_IDS);
    ...
    public int clearAcct(int aid, FaiList<Integer> unionPriIds){
        ...
        try {
            ...
            // send
            FaiBuffer sendBody = new FaiBuffer(true);
            unionPriIds.toBuffer(sendBody, StoreSalesSkuDto.Key.UNION_PRI_IDS);

            FaiProtocol sendProtocol = new FaiProtocol();
            sendProtocol.setCmd(MgProductStoreCmd.StoreSalesSkuCmd.CLEAR_ACCT);
            sendProtocol.setAid(aid);
            sendProtocol.addEncodeBody(sendBody);
            m_rt = send(sendProtocol);
            if (m_rt != Errno.OK) {
    ...
    /*public int migrateYKService(int aid, FaiList<Param> spuList) {
        ...
        try {
            ...
            if(spuList == null || spuList.isEmpty()){
                m_rt = Errno.ARGS_ERROR;
                Log.logErr(m_rt, "arg error;spuBizSummaryList is empty");
                return m_rt;
            }
            ...
            FaiProtocol sendProtocol = new FaiProtocol();
            sendProtocol.setCmd(MgProductStoreCmd.Cmd.MIGRATE_YK_SERVICE);
            sendProtocol.setAid(aid);
            sendProtocol.addEncodeBody(sendBody);
            m_rt = send(sendProtocol);
            if (m_rt != Errno.OK) {
   ...
   /* public int migrateYKStoreSku(int aid, FaiList<Param> storeSkuList) {
        ...
        try {
            ...
            if(storeSkuList == null || storeSkuList.isEmpty()){
                m_rt = Errno.ARGS_ERROR;
                Log.logErr(m_rt, "arg error;storeSkuList is empty;");
                return m_rt;
            }

            // send
            FaiBuffer sendBody = new FaiBuffer(true);
            storeSkuList.toBuffer(sendBody, StoreSalesSkuDto.Key.INFO_LIST, StoreSalesSkuDto.getInfoDto());

            FaiProtocol sendProtocol = new FaiProtocol();
            sendProtocol.setCmd(MgProductStoreCmd.Cmd.MIGRATE_YK_STORE_SKU);
            sendProtocol.setAid(aid);
            sendProtocol.addEncodeBody(sendBody);
            m_rt = send(sendProtocol);
            if (m_rt != Errno.OK) {
    ...
    /*public int migrateYKServiceDel(int aid, FaiList<Integer> pdIds) {
        ...
        try {
            ...
            if(pdIds == null || pdIds.isEmpty()){
                m_rt = Errno.ARGS_ERROR;
                Log.logErr(m_rt, "arg error;pdIds is empty;");
                return m_rt;
            }

            // send
            FaiBuffer sendBody = new FaiBuffer(true);
            pdIds.toBuffer(sendBody, StoreSalesSkuDto.Key.PD_IDS);

            FaiProtocol sendProtocol = new FaiProtocol();
            sendProtocol.setCmd(MgProductStoreCmd.Cmd.MIGRATE_YK_DEL);
            sendProtocol.setAid(aid);
            sendProtocol.addEncodeBody(sendBody);
            m_rt = send(sendProtocol);
            if (m_rt != Errno.OK) {
    ...
    public int restoreData(int aid, String xid, FaiList<Integer> pdIds) {
        ...
        try {
            ...
            FaiProtocol sendProtocol = new FaiProtocol();
            sendProtocol.setCmd(MgProductStoreCmd.Cmd.RESTORE_DATA);
            sendProtocol.setAid(aid);
            sendProtocol.addEncodeBody(sendBody);
            m_rt = send(sendProtocol);
            if (m_rt != Errno.OK) {
