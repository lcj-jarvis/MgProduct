...
public class ProductStoreProc extends AbstractProductProc{
    ...
    public int batchSetSkuStoreSales(int aid, String xid, int tid, int ownerUnionPriId, FaiList<Integer> unionPriIds, int pdId, int rlPdId, FaiList<ParamUpdater> updaterList, FaiList<Integer> softDelUnionPriIds) {
        return batchSetSkuStoreSales(aid, xid, tid, ownerUnionPriId, unionPriIds, pdId, rlPdId, ProductStoreValObj.SysType.PRODUCT, updaterList, softDelUnionPriIds);
    }

    /**
     * 修改sku库存销售信息
     */
    public int batchSetSkuStoreSales(int aid, String xid, int tid, int ownerUnionPriId, FaiList<Integer> unionPriIds, int pdId, int rlPdId, int sysType, FaiList<ParamUpdater> updaterList, FaiList<Integer> softDelUnionPriIds) {
        int rt = Errno.ERROR;
        if (m_cli == null) {
            rt = Errno.ERROR;
        ...
        rt = m_cli.batchSetSkuStoreSales(aid, xid, tid, ownerUnionPriId, unionPriIds, pdId, rlPdId, sysType, updaterList, softDelUnionPriIds);
        if (rt != Errno.OK) {
   ...
   /* public void migrateYKService(int aid, FaiList<Param> spuList) {
    ...
    public int getSpuBizSummaryInfoListByPdIdListAndUidList(int aid, int tid, FaiList<Integer> unionPriIdList, FaiList<Integer> pdIds, FaiList<Param> spuSalesStoreInfoList) {
        int rt;
        if (m_cli == null) {
            rt = Errno.ERROR;
            throw new MgException(rt, "get MgProductStoreCli error;flow=%d;aid=%d;", m_flow, aid);
        }
        rt = m_cli.getSpuBizSummaryInfoListByPdIdListAndUidList(aid, tid, unionPriIdList, pdIds, spuSalesStoreInfoList);
        if (rt != Errno.OK && rt != Errno.NOT_FOUND) {
            throw new MgException(rt, "getSpuBizSummaryInfoListByPdIdListAndUidList err;flow=%d;aid=%d;tid=%d;pdIds=%s;unionPriIds=%s", m_flow, aid, tid, pdIds, unionPriIdList);
        }
        return rt;
    }
